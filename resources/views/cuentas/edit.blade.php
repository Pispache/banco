<x-layouts.app>
    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        <div id="kt_content_container" class="container-xxl">
            <form class="form d-flex flex-column flex-lg-row" action="{{ isset($cuenta) ? route('cuentas.update', $cuenta->id) : route('cuentas.store') }}" enctype="multipart/form-data" method="POST">
                @csrf
                            ...
                            <div class="d-flex flex-column gap-5 gap-md-7">
                                <div class="d-flex flex-column flex-md-row gap-5">
                                    <div class="fv-row flex-row-fluid">
                                        ...
                                        <input class="form-control" name="numero" placeholder="Ingrese el número de cuenta" value="{{ old('numero', $cuenta->numero ?? '') }}" />
                                    </div>
                                    <div class="fv-row flex-row-fluid">
                                ...
                                <div class="d-flex flex-column flex-md-row gap-5">
                                    <div class="fv-row flex-row-fluid">
                                        ...
                                        <input class="form-control" type="number" step="0.01" name="saldo" value="{{ old('saldo', $cuenta->saldo ?? 0) }}" />
                                    </div>
                                    <div class="fv-row flex-row-fluid">
                                        <label class="required form-label">Cliente</label>
                                        <select class="form-control" name="user_id">
                                            @foreach($users as $user)
                                                <option value="{{ $user->id }}" {{ (old('user_id', $cuenta->user_id ?? '') == $user->id) ? 'selected' : '' }}>{{ $user->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
