<x-layouts.app>
    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        <div id="kt_content_container" class="container-xxl">
            <form class="form d-flex flex-column flex-lg-row" action="{{ isset($prestamo) ? route('prestamos.update', $prestamo) : route('prestamos.store') }}" enctype="multipart/form-data" method="POST">
                @csrf
                    ...
                    <div class="card card-flush py-4">
                        <div class="card-header">
                            <div class="card-title">
                                <h2>{{ isset($prestamo) ? 'Editar Préstamo' : 'Nuevo Préstamo' }}</h2>
                            </div>
                        </div>
                            ...
                            <div class="d-flex flex-column gap-5 gap-md-7">
                                <div class="d-flex flex-column flex-md-row gap-5">
                                    <div class="fv-row flex-row-fluid">
                                        ...
                                        <input class="form-control w-100" type="number" step="0.01" name="monto" value="{{ old('monto', $prestamo->monto ?? '') }}" />
                                    </div>
                                    <div class="fv-row flex-row-fluid">
                                        <label class="required form-label">Interés (%)</label>
                                        <input class="form-control w-100" type="number" step="0.01" name="interes" value="{{ old('interes', $prestamo->interes ?? '') }}" />
                                    </div>
                                </div>
                                <div class="d-flex flex-column flex-md-row gap-5">
                                    <div class="fv-row flex-row-fluid">
                                        <label class="required form-label">Plazo (meses)</label>
                                        <input class="form-control w-100" type="number" name="plazo" value="{{ old('plazo', $prestamo->plazo ?? '') }}" />
                                    </div>
                                    <div class="fv-row flex-row-fluid">
                                        <label class="required form-label">Cuenta</label>
                                        <select name="cuenta_id" class="form-control w-100">
